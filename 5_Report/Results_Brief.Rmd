...
df %>% select(prolific_participant_id, age, Sex) %>% distinct() %>%
    summarise(N = n(), Age = mean(age), `Female %` = mean(Sex=="Female"))
```
...
df %>% filter(!is.na(count_post_study_trial), Post_Topic != "Debrief") %>%
    select(prolific_participant_id, Post_Topic, response_Post_response) %>%
    group_by(Post_Topic) %>%
    count(response_Post_response)
...
Among ```r (df %>% select(prolific_participant_id) %>% unique() %>% dim())[1]``` participants, ```r round(mean(df %>% filter(!is.na(count_post_study_trial), Post_Topic == "Debrief") %>% select(response_Debrief_response) == "space")*100,2)``` % participants accurately answered the debrief.
...
ReadingTime_df <- df %>% filter(!is.na(count_count_board)) %>%
...
(verb_rt <- df %>% filter(!is.na(count_count_board)) %>% ## filter warm-up trials
    filter(probe_type == "v") %>%  ## isolate the verb probe responses
    filter(correct == 1) %>%
    group_by(prolific_participant_id, Dimension, Foreshadow, Shift) %>%
    summarise(Verb_rt = mean(response_time_Probe_response,na.rm =TRUE)) %>% as.data.frame() ) %>%
    group_by(Foreshadow, Shift) %>%
    summarise(RT = mean(Verb_rt))

(verb_acc <- df %>% filter(!is.na(count_count_board)) %>% ## filter warm-up trials
    filter(probe_type == "v") %>%  ## isolate the verb probe responses
##    filter(correct == 1) %>%
    group_by(prolific_participant_id, Dimension, Foreshadow, Shift) %>%
